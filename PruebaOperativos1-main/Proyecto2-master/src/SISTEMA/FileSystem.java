        ...
        if (dir != null) {
            ...
            while (actual != null) {
                if (actual.dato.name.equals(request.getFileName())) {
                    // 🔧 FIX: Try to access from buffer before deleting (may be hit or miss)
                    Nodo<Integer> actualBlock = actual.dato.blocks.getCabeza();
                    while (actualBlock != null) {
                        buffer.get(actualBlock.dato); // This will count hit or miss
                        actualBlock = actualBlock.siguiente;
                    }
        ...
        if (dir != null) {
            ...
            while (actual != null) {
                if (actual.dato.name.equals(request.getFileName())) {
                    ...
                    // 🔧 FIX: Check buffer first, then update
                    Nodo<Integer> actualBlock = actual.dato.blocks.getCabeza();
                    while (actualBlock != null) {
