...
public class FileSystem {
    ...
    private void executeCreate(IORequest request, IOProcess process) {
        ...
        if (dir != null) {
            ...
            if (allocatedBlocks != null) {
                ...
                // 🔧 FIX: Check buffer first (will be miss), then add to buffer
                Nodo<Integer> actualBlock = allocatedBlocks.getCabeza();
                while (actualBlock != null) {
                    // Try to get from buffer first (this will count as miss)
        ...
        if (dir != null) {
            ...
            while (actual != null) {
                if (actual.dato.name.equals(request.getFileName())) {
                    // 🔧 FIX: Just clean buffer without counting stats (delete doesn't need buffer access)
        ...
        if (dir != null) {
            ...
            while (actual != null) {
                if (actual.dato.name.equals(request.getFileName())) {
                    ...
                    // 🔧 FIX: Properly check buffer and count hits/misses for update
                    Nodo<Integer> actualBlock = actual.dato.blocks.getCabeza();
                    int hits = 0;
                    int misses = 0;
                    
                    while (actualBlock != null) {
                        CacheBlock cached = buffer.get(actualBlock.dato); // This counts hit or miss
                        if (cached != null) {
                            // Hit - update in buffer
                    ...
                    String cacheStatus = (hits > 0) ? " (" + hits + " HITs, " + misses + " MISSes)" : " (" + misses + " MISSes)";
                    AuditLog.registrarAccion(process.getUsuario(), "✏️ Actualizó el archivo '" + request.getFileName() + "'" + cacheStatus);
                    guardarEnArchivo();
                    return;
                }
                actual = actual.siguiente;
    ...
    private void executeRead(IORequest request, IOProcess process) {
        ...
        if (dir != null) {
            ...
            while (actual != null) {
                if (actual.dato.name.equals(request.getFileName())) {
                    // 🔧 FIX: Properly check buffer for each block
                    Nodo<Integer> actualBlock = actual.dato.blocks.getCabeza();
                    int hits = 0;
                    int misses = 0;
                    ...
                    while (actualBlock != null) {
                        ...
                        if (cached != null) {
                            ...
                            misses++;
                            // Add to buffer after miss
                            buffer.put(actualBlock.dato, request.getFileName(), new byte[1024]);
                        }
                        actualBlock = actualBlock.siguiente;
                    }
                    
                    String cacheStatus = (hits > 0) ? " (" + hits + " HITs, " + misses + " MISSes en buffer)" : " (" + misses + " MISSes - leyendo de disco)";
